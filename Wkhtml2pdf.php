...
class Wkhtml2pdf {
    ...
    public function __construct(ConfigInterface $config, ViewInterface $view) {
        $this->_binpath = base_path('plugins/nitmedia/wkhtmltopdf/lib/');
        $this->config   = $config;
        $this->_tmpfilepath = base_path('storage/sportmaniac/tmp/');
        $this->view     = $view;
        $this->_htmlfilepath = base_path('plugins/nitmedia/wkhtmltopdf/views/');
        
        $binpath = $this->config->get('nitmedia.wkhtmltopdf::binpath');

        if ($binpath) {
            if ($binpath[0] === '/') {
                $this->setBinPath($this->config->get('nitmedia.wkhtmltopdf::binpath'));
            } else {
                $this->setBinPath(realpath(__DIR__) . '/' . $this->config->get('nitmedia.wkhtmltopdf::binpath'));
            }
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::binfile')) {
            $this->setBinFile($this->config->get('nitmedia.wkhtmltopdf::binfile'));
        }

        /* Check the binary executable exists */
        $this->getBin();

        if ($this->config->get('nitmedia.wkhtmltopdf::html')) {
            $this->setHtml($this->config->get('html'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::orientation')) {
            $this->setOrientation($this->config->get('nitmedia.wkhtmltopdf::orientation'));
        } else {
            $this->setOrientation(self::ORIENTATION_PORTRAIT);
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::page_size')) {
            $this->setPageSize($this->config->get('nitmedia.wkhtmltopdf::page_size'));
        } else {
            $this->setPageSize(self::SIZE_A4);
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::toc')) {
            $this->setTOC($this->config->get('nitmedia.wkhtmltopdf::toc'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::grayscale')) {
            $this->setGrayscale($this->config->get('nitmedia.wkhtmltopdf::grayscale'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::title')) {
            $this->setTitle($this->config->get('nitmedia.wkhtmltopdf::title'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::debug')) {
            $this->debug = $this->config->get('nitmedia.wkhtmltopdf::debug');
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::header_html')) {
            $this->setHeaderHtml($this->config->get('nitmedia.wkhtmltopdf::header_html'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::footer_html')) {
            $this->setFooterHtml($this->config->get('nitmedia.wkhtmltopdf::footer_html'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::tmppath')) {
            $this->setTmpPath($this->config->get('nitmedia.wkhtmltopdf::tmppath'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::output_mode')) {
            $this->setOutputMode($this->config->get('nitmedia.wkhtmltopdf::output_mode'));
        }

        if ($this->config->get('nitmedia.wkhtmltopdf::options')) {
            $this->setOptions($this->config->get('nitmedia.wkhtmltopdf::options'));
        }
    }
    ...
    public function html($view, $data = array(), $name = 'file') {
        $this->setHtml(view()->file($this->_htmlfilepath.$view,$data));        
        return $this->output($this->getOutputMode(), $name . ".pdf");
    }
   
    public function htmlBarcode($view, $data = array(), $name = 'file') {
        $this->setHtml(view()->file($this->_htmlfilepath.$view,$data));
        $this->setBarcode();
        return $this->output($this->getOutputMode(), $name . ".pdf");
    }
    ...
    protected function _getCommand($in) {
        ...
        //barcode section
        $command .= $this->_have_headerhtml ? " --margin-top 0 --header-html " . escapeshellarg($this->_headerfilename) : "";
        $command .= $this->_have_footerhtml ? " --margin-bottom 0 --footer-html " . escapeshellarg($this->_footerfilename) : "";
        $command .= " -s A4";
        $command .= " --zoom 1";
        $command .= " --dpi 300";
        $command .= " -B 5mm";
